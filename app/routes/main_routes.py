...
from ..models import User, Post, db, post_shares, Notification
from ..forms import LoginForm, RegistrationForm, SharePostForm, UploadForm, FilterForm
from .. import csrf
...
def upload():
    ...
    if form.validate_on_submit():
        ...
        # 新闻摘要生成
        summary_output = news_summarizer(
            news_content, max_length=130, min_length=30, do_sample=False
        )
        summary_text = summary_output[0]["summary_text"]
        # sentiment analysis
        # emotion_scores = emotion_classifier(news_content)[0]
        emotion_scores = emotion_classifier(
            news_content, truncation=True, max_length=512
        )[0]
        ...
        sentiment_output = sentiment_classifier(
            news_content, truncation=True, max_length=512
        )
        ...
        post.emotions = {
            e["label"]: round(e["score"], 3) for e in emotion_scores_sorted
        }
        db.session.add(post)
        db.session.commit()
...
def share():
    ...
    for post in shared_posts:
        ...
        emotion_scores = emotion_classifier(post.body, truncation=True, max_length=512)[
            0
        ]
...
@main.route("/share_post/<int:post_id>", methods=["GET", "POST"])
@login_required
def share_post(post_id):
    ...
    ).all()  # Exclude the current user
    return render_template("share_post.html", post=post, users=users)


@main.route("/share_post_modal", methods=["POST"])
@login_required
def share_post_modal():
    ...
    existing_share = db.session.execute(
        sa.select(post_shares).where(
            post_shares.c.post_id == post.id, post_shares.c.user_id == user.id
        )
    ).first()
...
def user(username):
    ...
    posts = Post.query.filter(
        Post.user_id == current_user.id, Post.timestamp >= seven_days_ago
    ).all()
    ...
    return render_template(
        ...
        category_labels=(
            list(category_distribution.keys()) if category_distribution else []
        ),
        category_values=(
            list(category_distribution.values()) if category_distribution else []
        ),
...
def analysis():
    ...
    if request.method == "POST":
        ...
        emotion_result = (
            f"Emotion: {top_emotion['label']} ({top_emotion['score']:.2%} confidence)"
        )

        return render_template(
            "analysis.html", result=result, emotion_result=emotion_result
        )
...
@main.route("/mark_notification_read_and_redirect")
@login_required
